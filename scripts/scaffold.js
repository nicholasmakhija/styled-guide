...
export const renderHTML = (sheets, data, html) =>
  ...
  <head>
    ...
    <meta property="og:image:secure_url" content="https://styled.guide/assets/og-image.jpg">
    <meta property="og:image" content="https://styled.guide/assets/og-image.jpg">
  
    ${sheets}
    <script>var content = ${data};</script>
...
sync('src/app/**/index.html').map((file) => {
  ...
  const data = {
    currentPage: title,
    nav: array.map(({ title, sections }) => ({
      [title.toLowerCase()]: sections
    })),
    innerHTML: content
  };
